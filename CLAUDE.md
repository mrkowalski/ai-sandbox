...
- Shell scripts open with a comment block stating what the script guarantees and how it is invoked, use `set -euo pipefail`, and carry comments that explain *why* a step is ordered or written the way it is.
- Prefer failing loudly. Missing config and other discrepancies should result in a clear error message and exit.
...
- Code comments: none by default. Only for non-obvious invariants, workarounds, or external constraints. Never restate what the code does.
- Do not use emdash; use regular hyphens instead.
